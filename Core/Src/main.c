...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		if (can.rxFrameFlag)
		{
				can.rxFrameFlag = false;  // 清标志
				// 处理 can.rxData 中的数据
				printf("can.buffer[%d,%d,%d,%d,%d,%d,%d,%d]\n",can.rxData[0],can.rxData[2],can.rxData[3],can.rxData[4],can.rxData[5],can.rxData[6],can.rxData[7],can.rxData[8]);
		}
  }
  /* USER CODE END 3 */
}
